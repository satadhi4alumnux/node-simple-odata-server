...
// insert key at a perticular location
...
module.exports = function (cfg, req, res) {
  ...
  cfg.executeQuery(queryOptions.collection, queryOptions, req, function (err, result) {
    ...
    if (queryOptions.$skiptoken) {
      const nextLink = cfg.serviceUrl + '/' + req.params.collection + (sAdditionIntoContext === "" ? `?\$skiptoken=${queryOptions.$skiptoken}` : `&\$skiptoken=${queryOptions.$skiptoken}`);
      out = insertKey('@odata.nextLink', nextLink, out, 1);
    }
